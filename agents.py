...
def cat_agent(message):
    system_message = '''You are a helpful assistant that catergorizes the given message into three catergories
...
def drug_id(message,history):
    ...
    if 'paracetamol' in drugs.lower() :
        return paracetamol_agent(message,history)
    if 'insulin' in drugs.lower() :
        return insulin_agent(message,history)    
    else :
